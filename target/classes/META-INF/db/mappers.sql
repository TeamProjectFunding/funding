...
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT EP.*, u.userprofileimage, U.USERNAME FROM EVENTPRIZE EP, USERS U, EVENT E WHERE EP.USERID=U.USERID AND E.EVENTNUMBER=ep.eventnumber ORDER BY EVENTPRIZENUMBER) A) WHERE RN BETWEEN 1 AND 5;