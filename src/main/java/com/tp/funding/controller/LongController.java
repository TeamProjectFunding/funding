...
public class LongController {
	...
	public String goodsInfoNavigation(int fundingCode, String infoType, String pageNum, Model model) {
		if (infoType.equals("goodsViewNews")) {// 새소식
			...
			model.addAttribute("fundingPeopleCount",
					fundingGoodsService.fundingDetail(fundingCode).getFundingPeopleCount());
	...
	public String fundingApply(FundingGoods fundingGoods, MultipartHttpServletRequest mRequest, Model model) {
		if (repeatF5) {
			...
			Notification notification = new Notification(
					fundingGoods.getFundingName() + " 펀딩신청 되었습니다. 관리자 심사 후 승인여부 안내드립니다. 신청해주셔서 감사드립니다.", "admin",
					companyId, null);
	...
	public String fundingComplate(FundingGoodsDetail fundingGoodsDetail, String userId, int fundingCategory,
		...
		DepositAndWithdrawal depositAndWithdrawal = new DepositAndWithdrawal(1, changeMoneyAmount * -1,
				user.getUserAccountBalance(),
				fundingGoodsService.fundingDetail(fundingCode).getFundingName() + " 상품에 " + changeMoneyAmount + "원 펀딩",
				null, userId, rewardCode);
	...
	public String fundingDetailView(int fundingCode, String companyId, Model model) {
		...
		if (good.getFundingCategory() == 0) { // 투자
			...
			model.addAttribute("reward", reward);
			model.addAttribute("companyDNWList",
					depositAndWithdrawalService.companyRewardDNWList(companyId, reward.getRewardCode()));
		} else {// 리워드
			model.addAttribute("rewardList", rewardService.fundingRewardList(fundingCode));
		}
	...
	public String fundingDetailUserView(int fundingCode, int fundingGoodsDetailNumber, String userId, Model model) {
		...
		if (good.getFundingCategory() == 0) { // 투자
			Reward reward = rewardService.fundingRewardList(fundingCode).get(0);
			model.addAttribute("reward", reward);
			model.addAttribute("userRewardDNWList",
					depositAndWithdrawalService.userRewardDNWList(userId, reward.getRewardCode()));
		} else {// 리워드
	...
	public String myPageMain(String bank, String accountHolder, String accountNumber, String userId, String companyId,
		...
		if (userId != null && !userId.equals("")) {
			...
			model.addAttribute("myPostTotalCnt", qnAService.myTotQna(userId)
					+ fundingQuestionService.myFundingtotQna(userId) + fgCommentsService.myFundingTotComments(userId));
			...
			model.addAttribute("companyEndFundingCount", fundingGoodsService.companyEndFundingCount(companyId)); // 회사가
																													// 총
																													// 펀딩한
																													// 갯수
	...
	public String depositMypage(DepositAndWithdrawal depositAndWithdrawal, Model model, HttpSession session) {
		...
		if (userId != null & !userId.equals("")) {// 유저
			depositAndWithdrawal.setdNWBalance(
					usersService.userDetail(userId).getUserAccountBalance() + depositAndWithdrawal.getdNWAmount());
			...
			depositAndWithdrawal.setdNWBalance(companyService.companyDetail(companyId).getCompanyAccountBalance()
					+ depositAndWithdrawal.getdNWAmount());
			depositAndWithdrawalService.writeDepositAndWithdrawal(depositAndWithdrawal);
			Company company = new Company();
			company.setCompanyId(companyId);
	...
	public String doInterestPayment(String adminId, String companyId, double changeMoneyDouble, int rewardCode,
			Model model) {
		...
		notificationService.notificationWrite(new Notification(
				reward.getRewardName() + " 상품 이자 지급(총 " + reward.getFundingInvestmentPeriod() + "회/ "
						+ (reward.getInvestmentReceiveCount() + 1) + "회 지급)  지급 금액: " + changeMoney + "원",
				adminId, companyId, null)); // 회사 알림창 증가
		DepositAndWithdrawal depositAndWithdrawal = new DepositAndWithdrawal(1, changeMoney * -1,
				companyService.companyDetail(companyId).getCompanyAccountBalance(),
				reward.getRewardName() + " 상품 고객에게 이자 지급", companyId, null, rewardCode);
		...
		List<FundingGoodsDetail> fundingDetailList = fundingDetailService.interestPaymentList(reward.getFundingCode()); 
		...
		for (int i = 0; i < fundingDetailList.size(); i++) {
			...
			int dNWAmount = fundingDetailList.get(i).getFundingAmount() * rewardInterst / 1200; // 투자한금액*이자율/100/12(개월)
																								// 이자받을금액
			...
			DepositAndWithdrawal userBalancePlus = new DepositAndWithdrawal(0, dNWAmount, dNWBalance,
					reward.getRewardName() + " 상품 이자 지급", null, userId, rewardCode);
			depositAndWithdrawalService.writeDepositAndWithdrawal(userBalancePlus); // 입출금 정보 등록 (유저가 입금받음)
			notificationService.notificationWrite(new Notification(
					reward.getRewardName() + " 상품 이자 " + dNWAmount + "원 지급하엿습니다.", adminId, userId, null)); // 유저 알림 작성
			...
			if (principalPayment) { // 원금 지급로직
				...
				DepositAndWithdrawal userBalancePlusPrincipal = new DepositAndWithdrawal(0, fundingAmount,
						dNWBalanceTemp, reward.getRewardName() + " 상품 원금 지급", null, userId, rewardCode);
				depositAndWithdrawalService.writeDepositAndWithdrawal(userBalancePlusPrincipal); // 입출금 정보 등록 (유저가 입금받음)
				notificationService.notificationWrite(new Notification(
						reward.getRewardName() + " 상품 원금 " + fundingAmount + "원 지급하엿습니다.", adminId, userId, null)); // 유저
																													// 알림
																													// 작성
			}
		}
		if (principalPayment) {// 원금 지급일 때
			...
			companyService.companyBalanceModify(company); // 회사 잔액 감소
			notificationService.notificationWrite(new Notification(
					reward.getRewardName() + " 상품 펀딩 종료로 고객에게 원금 지급 총 금액: " + principalTotalSum + "원이 계좌에서 출금되었습니다.",
					adminId, companyId, null)); // 회사 알림창 증가
			DepositAndWithdrawal tempDepositAndWithdrawal = new DepositAndWithdrawal(1, principalTotalSum * -1,
					companyService.companyDetail(companyId).getCompanyAccountBalance(),
					reward.getRewardName() + " 상품 펀딩 종료로 고객에게 원금 지급", companyId, null, rewardCode);
