...
INSERT INTO Notification VALUES (NOTIFICATIONNUMBER.nextval,'알림내용','2020-06-07',0,'user','admin',1);
...
--company full query
SELECT COM.*, (SELECT count(*) FROM Company C, notification N where c.companyid=n.companyid) NotificationCount FROM COMPANY COM;
...
SELECT COM.*, (SELECT count(*) FROM Company C, notification N where c.companyid=n.companyid) NotificationCount FROM COMPANY COM;
--EVENTPRIZE정보
SELECT A.* , EP.* FROM (SELECT userProfileImage, userName FROM EVENTPRIZE E, USERS U WHERE E.USERID=U.USERID) EP, EVENTPRIZE A;
--EVENTREPLY정보
SELECT A.*, ER.* FROM (SELECT userProfileImage, userName FROM EVENTREPLY E, USERS U WHERE E.USERID=U.USERID) ER, EVENTREPLY A;
